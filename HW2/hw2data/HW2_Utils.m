...
% Last modified: 11-Feb-2016
    ...
    methods (Static)
        ...
        function [rects,feats]= detect(im, w, b, shldDisplay)
            ...
            for s = 1:length(scales) % consider each scale in turn
                ...
                [topLefts_s, scores_s, feats_s] = deal(cell(1, nBatch));
                for i=1:nBatch
                    [feats_s{i}, topLefts_s{i}] = obj.getBatch(i);
                    D = HW2_Utils.l2Norm(feats_s{i});
                    scores_s{i} = D'*w + b;
                end;
                feats_s    = cat(2, feats_s{:});
            ...
            [rects, pick] = HW2_Utils.nms(rects, 0.5);
            feats=feats(:,pick); 
        ...
        function genRsltFile(w, b, dataset, outFile)
            ...
            for i=1:nIm
                ...
                im = imread(imFiles{i});
                [rects{i},feats{i}] = HW2_Utils.detect(im, w, b);
            end
            save(outFile, 'rects','feats');
